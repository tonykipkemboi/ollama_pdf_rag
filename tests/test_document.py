...
@pytest.mark.skipif(
    not Path("data/pdfs/sample/scammer-agent.pdf").exists(),
...
def test_split_documents(processor):
    ...
    doc = Document(
        page_content="This is a test document " * 1000, metadata={"source": "test"}
    )
    documents = [doc]
...
def test_metadata_preservation(processor):
    ...
    doc = Document(page_content="This is a test document " * 1000, metadata=metadata)
    documents = [doc]
    chunks = processor.split_documents(documents)
